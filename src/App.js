...
class App extends Component {
    ...
    moveDevice = (deviceIndex) => {
        ...
        // why dis code ^^^^^^^^^ console.log('2. '+ this.countGarageDevices())
        if (this.countGarageDevices() <= 10) {
            this.setState({
                devices: moveDevices
            }, () => {
    ...
    removeDevice = (deviceIndex) => this.setState({
        devices: this
            .state
            .devices
            .slice(deviceIndex, deviceIndex + 1)
    })
    ...
    render() {
                            ...
                            return (
                                <div>
                                    <Device
                                        ...
                                        inputChangeHandler={(e) => inputHandler(e,index)}
                                        move=
                                        {()=>this.moveDevice(index)}
                                        delete
                                        ={() => this.removeDevice(index)}
                                        edit={() => {
                                        console.log(`edit button has been clicked for index ${index}`)
                                    }}></Device>
                                </div>
                            )
                        }
        ...
        if (this.countGarageDevices() == 10) {
            maxStorage = (
                <h1 className={classes.AlertDisplay}>Maximum Storage Capacity Reached!</h1>
            )
        }
        ...
        return (
            <div>
                ...
                <div className="notInGarage">
                    <h2>Has been Checked out</h2>
                    {checkoutDevices}
                </div>
